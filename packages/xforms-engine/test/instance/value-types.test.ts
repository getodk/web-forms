		...
		describe('type="int"', () => {
			...
			describe('setting int values', () => {
				...
				describe.each<SetIntInputValueCase>([
					...
					{ inputType: 'number', inputValue: 10.1, expectedValue: 10n, expectFailure: true },
				])('setValue($inputType)', ({ inputValue, expectedValue, expectFailure }) => {
					let testFn: typeof it | typeof it.fails;

					if (expectFailure) {
						testFn = it.fails;

						// This is intentionally left as a lint error so it will fail CI if
						// the case hasn't been fixed.
						console.error(
							'Fix this `IntInputNode.setValue` case before merge!',
							'input value:',
							inputValue,
							'expected value:',
							expectedValue
					...
					testFn(`sets ${inputValue}, resulting in value ${expectedValue}`, () => {
