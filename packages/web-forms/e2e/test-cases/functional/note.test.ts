...
test.describe('Note Question Type', () => {
	...
	test('renders all possible note types', async ({ context }) => {
		...
		await formPage.text.expectLabel('This form illustrates the note concept in ODK. This is not a concept that exists in the underlying ODK XForms spec, it\'s introduced by XLSForm and discussed informally. Typically a note only has a label.');
