...
import type { GeopointAccuracyThresholdOptions } from '@/lib/geopoint/GeopointAccuracyThresholdOptions.ts';
			...
			<div class="geo-dialog-body">
				...
				<div class="geopoint-information">
					...
					<strong v-if="accuracy.value != null" class="geo-quality">
						{{ accuracy.value }}m
						-
						<GeopointAccuracyQuality :accuracy="accuracy" />
					</strong>
					...
					<p v-if="previousAccuracy.value">
						Previous saved location at {{ accuracy.value }}m
					</p>
				</div>
			</div>
		</template>
...
// TODO: many of these styles are extracted (duplicated) from `InputGeopoint.vue`
// How much of this is styling `GeopointFormattedValue`??
.geo-dialog-body {
	display: flex;
...
// Note: separate block is defined so the above TODO does not apply to these
// styles, which were also extracted but do not duplicate anything.
.geo-dialog-body {
